...
#	$self->{schedule} = LoadFile($fn);
#	$self->shift_schedule( - 3600 - 20 * 60 + 25 * 60);
}
...
#					$self->log("skip entry ".uts($entry->{_start_time})." = ".cts($entry->{_start_time})." ".$entry->{source_uri});
					next;
...
#		$self->log($self->melted->append($next_entry->{source_uri}, 0, $next_entry->{source_duration}));
...
#		$self->log($self->melted->wipe()); DO NOT UNCOMMENT!
	} elsif (defined $next_block) {
#		$self->log("   - next [d]block ".$next_block->{name})
		$next_entry = $next_block->{entries}->[0];
